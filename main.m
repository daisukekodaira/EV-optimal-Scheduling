...
% fsoc: Target SOC for EVs [%]
% Power: PCS power[kw]. Chargeing and discharging. Values are refered to KIAPI fast charger
EV_num=20; 
EV_cap=60;
boundary=1800;
Target_SOC=0.8;
...
%          There are 3 scenarios as mean, upper boudanry and lower boundary
...
%      one of the scenarios is selected to reduce the calculation time for now
w_t=1.1;
...
[x,fval,exitflag,in,out,op_size,energy,mat_a1,mat_b1]=QP_test(B_data_sample,plug_in,ori_soc, EV_num,EV_cap,boundary,Tariff_B,Target_SOC,PCS_power,w_t);
% % for debug --------------------------------------------------------
% % x=round(x,2);
% sum(mat_a1*x==mat_b1)
% % -------------------------------------------------------------------
